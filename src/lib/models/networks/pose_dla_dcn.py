...
class IDAUp(nn.Module):
    ...
    def forward(self, layers, startp, endp):
        for i in range(startp + 1, endp):
            ...
            #print(i, startp, endp, upsample,project)
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        assert down_ratio in [2, 4, 8, 16, 32]
        ...
        self.ida_up = IDAUp(out_channel, channels[self.first_level:6],
                            [2 ** i for i in range(4)])
        #print(channels[self.first_level], self.first_level,len(channels[self.first_level:]),channels[self.first_level:6])
        #64 2 4 [64, 128, 256,512]
        self.heads = heads
        # self.head_fusion1 = nn.Sequential(nn.Conv2d(self.heads['hm'], self.heads['wh'], 1, 1))
        # self.head_fusion2 = nn.Sequential(nn.Conv2d(self.heads['wh'], self.heads['hm'], 1, 1))
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        z = {}
        # z['hm'] = self.__getattr__('hm')(x)
        # hm = self.head_fusion1(z['hm'])
        # z['reg'] = self.__getattr__('reg')(x)
        # z['wh'] = self.__getattr__('wh')(x)
        # wh = self.head_fusion2(z['wh'])
        # z['hm'] = z['hm'] + wh
        # z['wh'] = z['wh'] + hm
        for head in self.heads:
